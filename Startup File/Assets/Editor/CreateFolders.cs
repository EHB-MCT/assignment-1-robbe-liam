...
public class CreateFolders : EditorWindow
{
    ...
    {
        CreateFolders window = ScriptableObject.
        CreateInstance<CreateFolders>();
        window.position = new Rect(Screen.width / 2, Screen.height /
        2, 400, 150);
        window.ShowPopup();
    }
...
"Icon"
        ...
        {
            if (!Directory.Exists("Assets/" + projectName + "/UI/" +
            subfolder))
            {
    ...
    void OnGUI()
    {
        ...
        projectName = EditorGUILayout.TextField("Project Name: ",
        projectName);
        this.Repaint();
        GUILayout.Space(70);
